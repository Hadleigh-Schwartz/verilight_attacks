...
def preprocess_face_for_landmark_detection(img):
    """
    img: tensor of cropped face
    """
    # preprocess img
    proc_img = pad_image(img, desired_size=192) # resize
    # if VIS:
...
def unnormalize_face(img):
    """
    img: tensor of cropped face
    """
    # reverse preprocess img
    img = img.permute(1, 2, 0)
    img = (img + 1.0) * 127.5
    return img

def unnormalize_eye(img):
    """
    img: tensor of cropped eye
    """
    # reverse preprocess img
    img *= 255
    img = img.permute(1, 2, 0)
...
def preprocess_eye_for_landmark_detection(img):
    """
    img: tensor of cropped eye
    """
    # preprocess img
...
def get_eye_bounds(facial_landmarks, eye = "left"):
    """
    ...
    rightEyeUpper0 = facial_landmarks[rightEyeUpper0_ids, :]
...
def vis_facial_landmarks(img, facial_landmarks):
    """
...
def vis_eye_cropping(img, facial_landmarks, right_eye_bounds, left_eye_bounds):
    """
    img: tensor, H x W x 3, RGB
    """
    img = img.numpy().astype(np.uint8)
...
def vis_iris_landmarks_on_eye_crop(img, iris_landmarks):
    """
...
def vis_iris_landmarks_on_face(img, iris_landmarks):
    """
...
def iris_landmarks_to_original_pixel_space(iris_landmarks, eye_bounds, eye_crop_dims):
    """
    The iris landmarks were computed on a 64x64 padded image of the eye crop, obtained by extracting a crop from the 192x192
    and then resizing this according to pad_image. 
...
def compare_to_real_mediapipe(landmarks, blendshapes):
    real_mp_landmarks, real_mp_blendshapes = get_real_mediapipe_results("padded_face.png")
    padded_face = cv2.imread("padded_face.png")
...
face = detect_face(img_tensor)
proc_face = preprocess_face_for_landmark_detection(face)

# run facial landmark detection
facial_landmarks, confidence = facelandmarker.predict(proc_face) # predict
...
padded_face = unnormalize_face(proc_face) # undo normalization and permutation applied for facial landmark detection, but leave the padding it added
vis_facial_landmarks(padded_face, facial_landmarks)

# get eye bounds
right_eye_left, right_eye_right, right_eye_top, right_eye_bottom, right_eye_width, right_eye_height = get_eye_bounds(facial_landmarks, eye = "right")
left_eye_left, left_eye_right, left_eye_top, left_eye_bottom, left_eye_width, left_eye_height = get_eye_bounds(facial_landmarks, eye = "left")
...
# pad the eye crops
proc_left_eye_crop = preprocess_eye_for_landmark_detection(left_eye_crop)
proc_right_eye_crop = preprocess_eye_for_landmark_detection(right_eye_crop)

# run iris detection
left_eye_contour_landmarks, left_iris_landmarks = irislandmarker.predict(proc_left_eye_crop)
right_eye_contour_landmarks, right_iris_landmarks = irislandmarker.predict(proc_right_eye_crop)
...
left_iris_landmarks = iris_landmarks_to_original_pixel_space(left_iris_landmarks, (left_eye_left, left_eye_right, left_eye_top, left_eye_bottom), (left_eye_width, left_eye_height))
right_iris_landmarks = iris_landmarks_to_original_pixel_space(right_iris_landmarks, (right_eye_left, right_eye_right, right_eye_top, right_eye_bottom), (right_eye_width, right_eye_height))

# only for visualization
vis_iris_landmarks_on_face(padded_face, left_iris_landmarks)
vis_iris_landmarks_on_face(padded_face, right_iris_landmarks)
