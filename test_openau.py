...
class AU():
    ...
    def draw_text(self, img, words, probs):
        if img.shape[0] < 2000:    # make image larger so that text is actually visible
            scale = int(2000 / img.shape[0])
...
while True:
    ...
    pred = au.predict(frame_tensor)
    vis = au.visualize(frame, pred)
    cv2.imshow('frame', cv2.cvtColor(vis, cv2.COLOR_RGB2BGR))
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

# # load an image
# img = cv2.imread('obama.jpeg')
# # convert to RGb
# img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
# # convert to tensor for consistency with how it will ultimately
# # be passed to predict() in the loss function
# img_tensor = torch.tensor(img, dtype=torch.float32).permute(2, 0, 1)
# pred = au.predict(img_tensor)
# au.visualize(img, pred)
